...
- (void)didPan:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    CGPoint translatedPoint = [gestureRecognizer translationInView:""_container];
    if(gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        
        /// Save the inital Y to reuse later
        initialCenterY = ""_container.center.y;
        ...
        [UIView animateWithDuration:0.3 animations: ^ {
            ""_container.center = CGPointMake(""_container.center.x, initialCenterY);
        }];
        CZLog(@"SOLWeatherView", @"Did End Panning");
        
    } else if(translatedPoint.y <= 50 && translatedPoint.y > 0) {
        /// Translate the container
        ""_container.center = CGPointMake(""_container.center.x, self.center.y + translatedPoint.y);
    }
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    if([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        CGPoint velocity = [panGestureRecognizer velocityInView:""_container];
...
- (void)initializeForecastDayLabels
{
    ...
    for(int i = 0; i < [forecastDayLabels count]; ++i) {
        ...
        [""_container addSubview:forecastDayLabel];
    }
}
...
- (void)initializeForecastIconLabels
{
    ...
    for(int i = 0; i < [forecastIconLabels count]; ++i) {
        ...
        [""_container addSubview:forecastIconLabel];
    }
}
