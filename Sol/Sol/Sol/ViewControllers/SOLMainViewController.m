...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        ...
        if([SOLStateManager weatherData]) {
            ""_weatherData = [NSMutableDictionary dictionaryWithDictionary:[SOLStateManager weatherData]];
        } else {
            ""_weatherData = [NSMutableDictionary dictionaryWithCapacity:5];
        }
        
        /// Initialize the weather tags array with saved data, if it exists
        if([SOLStateManager weatherTags]) {
            ""_weatherTags = [NSMutableArray arrayWithArray:[SOLStateManager weatherTags]];
        } else {
            ""_weatherTags = [NSMutableArray arrayWithCapacity:4];
        }
        
        ""_dateFormatter = [[NSDateFormatter alloc]init];
        [""_dateFormatter setDateFormat:@"EEE MMM d, h:mm a"];
        ...
        if([""_weatherData count] >= kMAX_NUM_WEATHER_VIEWS) {
...
- (void)initializeViewControllers
{
    ...
    ""_addLocationViewController = [[SOLAddLocationViewController alloc]initWithNibName:nil bundle:nil];
    ""_addLocationViewController.delegate = self;
    
    /// Initialize the settings view controller
    ""_settingsViewController = [[SOLSettingsViewController alloc]initWithNibName:nil bundle:nil];
...
- (void)initializeSubviews
{
    ...
    ""_darkenedBackgroundView = [[UIView alloc]initWithFrame:self.view.bounds];
    [""_darkenedBackgroundView setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.5]];
    [self.view addSubview:_darkenedBackgroundView];
    
    /// Initialize the Sol° logo label
    ""_solLogoLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 160, 160)];
    ""_solLogoLabel.center = CGPointMake(self.view.center.x, 0.5 * self.view.center.y);
    ""_solLogoLabel.font = [UIFont fontWithName:CLIMACONS_FONT size:200];
    ""_solLogoLabel.backgroundColor = [UIColor clearColor];
    ""_solLogoLabel.textColor = [UIColor whiteColor];
    ""_solLogoLabel.textAlignment = NSTextAlignmentCenter;
    ""_solLogoLabel.text = [NSString stringWithFormat:@"%c", ClimaconSun];
    [self.view addSubview:_solLogoLabel];

    /// Initialize the Sol° title label
    ""_solTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 64)];
    ""_solTitleLabel.center = CGPointMake(self.view.center.x, self.view.center.y);
    ""_solTitleLabel.font = [UIFont fontWithName:@"HelveticaNeue-UltraLight" size:64];
    ""_solTitleLabel.backgroundColor = [UIColor clearColor];
    ""_solTitleLabel.textColor = [UIColor whiteColor];
    ""_solTitleLabel.textAlignment = NSTextAlignmentCenter;
    ""_solTitleLabel.text = @"Sol°";
    [self.view addSubview:_solTitleLabel];

    /// Initialize the paging scroll wiew
    ""_pagingScrollView = [[SOLPagingScrollView alloc]initWithFrame:self.view.bounds];
    ""_pagingScrollView.delegate = self;
    [self.view addSubview:""_pagingScrollView];
    
    /// Initialize the page control
    ""_pageControl = [[UIPageControl alloc]initWithFrame: CGRectMake(0, self.view.bounds.size.height - 32,
...
- (void)initializeSettingsButton
{
    ...
    [""_settingsButton setFrame:CGRectMake(4, self.view.bounds.size.height - 48, 44, 44)];
...
- (void)initializeLocalWeatherView
{
    ...
    SOLWeatherData *localWeatherData = [""_weatherData objectForKey:[NSNumber numberWithInteger:kLOCAL_WEATHER_VIEW_TAG]];
    if(localWeatherData) {
...
- (void)initializeNonlocalWeatherViews
{
    for(NSNumber *tagNumber in ""_weatherTags) {
        ...
        SOLWeatherData *weatherData = [""_weatherData objectForKey:tagNumber];
        if(weatherData) {
...
- (void)updateWeatherData
{
    ...
    for(SOLWeatherView *weatherView in ""_pagingScrollView.subviews) {
        if(weatherView.local == NO) {
            
            /// Only update non local weather data
            SOLWeatherData *weatherData = [""_weatherData objectForKey:[NSNumber numberWithInteger:weatherView.tag]];
            ...
            if([[NSDate date]timeIntervalSinceDate:weatherData.timestamp] >= kMIN_TIME_SINCE_UPDATE || !weatherView.hasData) {
                ...
                [[SOLWundergroundDownloader sharedDownloader]dataForPlacemark:weatherData.placemark withTag:weatherView.tag completion:^(SOLWeatherData *data) {
                    if (data) {
                        ...
                        // Update Weather View
                        [""_weatherData setObject:data forKey:[NSNumber numberWithInt:weatherView.tag]];
                        [self updateWeatherView:weatherView withData:data];
                        
                        /// Save the downloaded data
                        [SOLStateManager setWeatherData:""_weatherData];
                        if([""_weatherData count] >= kMAX_NUM_WEATHER_VIEWS) {
                            self.addLocationButton.hidden = YES;
                        }
                    }
...
- (void)updateWeatherView:(SOLWeatherView *)weatherView withData:(SOLWeatherData *)data
{
    ...
    /// Set the update time
    weatherView.updatedLabel.text = [NSString stringWithFormat:@"Updated %@", [""_dateFormatter stringFromDate:data.timestamp]];
...
- (void)addLocationButtonPressed
{
    ...
    if([_pagingScrollView.subviews count] > 0) {
        ...
        [UIView animateWithDuration:0.3 animations: ^ {
            ""_solLogoLabel.alpha = 0.0;
            ""_solTitleLabel.alpha = 0.0;
        }];
    }
...
- (void)didAddLocationWithPlacemark:(CLPlacemark *)placemark
{
    ...
    SOLWeatherData *weatherData = [""_weatherData objectForKey:[NSNumber numberWithInteger:placemark.locality.hash]];
    ...
    if(!weatherData) {
        ...
        [SOLStateManager setWeatherTags:""_weatherTags];
    ...
    if([""_pagingScrollView.subviews count] >= kMAX_NUM_WEATHER_VIEWS) {
        self.addLocationButton.hidden = YES;
    }
}
...
- (void)dismissAddLocationViewController
{
    ...
    [UIView animateWithDuration:0.3 animations: ^ {
        ""_solLogoLabel.alpha   = 1.0;
        ""_solTitleLabel.alpha  = 1.0;
    }];
...
- (void)settingsButtonPressed
{
    ...
    ""_settingsViewController.locations = locations;
...
- (void)didMoveWeatherViewAtIndex:(NSInteger)sourceIndex toIndex:(NSInteger)destinationIndex
{
    ...
    if([""_weatherData objectForKey:[NSNumber numberWithInteger:kLOCAL_WEATHER_VIEW_TAG]]) {
        sourceIndex +=1 ;
        destinationIndex += 1;
    }
...
- (void)didRemoveWeatherViewWithTag:(NSInteger)tag
{
    ...
    for(SOLWeatherView *weatherView in ""_pagingScrollView.subviews) {
        if(weatherView.tag == tag) {
            [""_pagingScrollView removeSubview:weatherView];
            ""_pageControl.numberOfPages -= 1;
        }
    }
...
- (void)downloadDidFinishWithData:(SOLWeatherData *)data withTag:(NSInteger)tag
{
    ...
    for(SOLWeatherView *weatherView in ""_pagingScrollView.subviews) {
        if(weatherView.tag == tag) {
            ...
            [""_weatherData setObject:data forKey:[NSNumber numberWithInt:tag]];
            [self updateWeatherView:weatherView withData:data];
            [weatherView.activityIndicator stopAnimating];
        }
    }
